...
void printBoard(Player p1, Player p2) {
    ...
    for (int i = 0; i <= 5; ++i) grid[0][i] = "[ " + string(1, BOARD_LAYOUT[i]) + " ]";
    // Columna derecha (casillas 6-9)
    for (int i = 0; i < 4; ++i) grid[1 + i][5] = "[ " + string(1, BOARD_LAYOUT[6 + i]) + " ]";
    // Fila inferior (casillas 10-15), se mapea en reversa para el dibujado.
    for (int i = 0; i <= 5; ++i) grid[5][5 - i] = "[ " + string(1, BOARD_LAYOUT[10 + i]) + " ]";
    // Columna izquierda (casillas 16-19), se mapea en reversa.
    for (int i = 0; i < 4; ++i) grid[4 - i][0] = "[ " + string(1, BOARD_LAYOUT[16 + i]) + " ]";
